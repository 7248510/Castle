...
//Crash course in C++ Boost get request
...
std::string request(std::string host, boost::asio::io_context& io_context, std::string portNum) {
    ...
    const auto endpoints = resolver.resolve(host, portNum);
...
int main()
{
    ...
    try {
        const auto response = request("127.0.0.1", io_context, "8080");
